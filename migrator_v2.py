...
class FirebirdToPgMigrator:
    ...
    def _load_table(self, tbl_cfg: dict, dry_run: bool):
        ...
        try:
            ...
            if not self._shutdown or completed:
...
def main():
    ...
    if not os.path.exists(config_file):
        print(f"\n[MIGRATOR V2] Erro: Arquivo de configuração não encontrado: {config_file}")
        sys.exit(1)

    if not args.table and not args.dry_run:
        print("\n[MIGRATOR V2] Erro: Você precisa informar a tabela (--table).")
        print("\nExemplos de execução ADHOC:")
        print(f"  uv run {sys.argv[0]} --work-dir MIGRACAO_0001 --table NOME_DA_TABELA")
        print(f"  uv run {sys.argv[0]} --work-dir MIGRACAO_0001 --table NOME_DA_TABELA --reset")
        sys.exit(1)
