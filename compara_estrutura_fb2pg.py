...
entre Firebird e PostgreSQL. Diferenças de tipo são reportadas como [WARNING-TIPO].
...
def _strip_outer_parens(expr: str) -> str:
    """Remove parênteses externos balanceados de uma expressão."""
    while True:
        expr = expr.strip()
...
def _normalize_check_expr(expr: str) -> str:
    """Normaliza expressão CHECK para comparação semântica entre FB e PG."""
    import re
    e = (expr or '').lower()
    ...
    if m2:
        e = m2.group(1).strip()
    # Remove type casts do PG: ::integer, ::double precision, ::character varying, etc.
    e = re.sub(r'::[a-z][a-z0-9]*(?:\s+[a-z][a-z0-9]*)?', '', e)
    # Remove parênteses em torno de literais numéricos: (0) → 0
    e = re.sub(r'\((\d+(?:\.\d+)?)\)', r'\1', e)
    # Remove parênteses externos redundantes (PG adiciona camada extra ao armazenar)
    e = _strip_outer_parens(e)
    e = re.sub(r'\s+', ' ', e).strip()
    return e
